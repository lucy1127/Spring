...
public class TrainTicket {
    ...
    public void checkParameter(CreateTrainTicketRequest request, CheckErrorException e) {
        if (null == request.getTrainNo()) {
            e.getCheckError().add(ErrorCode.TrainNoIsEmpty);
        }
        if (request.getFromStop().isEmpty()) {
            e.getCheckError().add(ErrorCode.TrainFromStopIsEmpty);
        }
        if (request.getToStop().isEmpty()) {
            e.getCheckError().add(ErrorCode.TrainToStopIsEmpty);
        }
    }
    ...
    public void checkDate(String date,CheckErrorException e) {
        ...
        if (!matcher.matches()) {
            e.getCheckError().add(ErrorCode.TakeDate);
        }
    }
